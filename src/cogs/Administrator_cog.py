...
from src.constants.Constants import BOT_COMMANDER_ROLE
...
class Administrator_cog(commands.Cog, name='administrator commands'):
    ...
    async def clear(self, ctx, limit: int = 9999):
        ...
        print("command: /clear  received from \'%s\' on guild \'%s\' channel \'%s\'" % (ctx.message.author.display_name, ctx.channel.guild.name, ctx.channel.name))
    ...
    async def post(self, ctx: commands.Context, *, arguments: str):
        ...
        print("command: /post received from \'%s\' on guild \'%s\' channel \'%s\'" % (ctx.message.author.display_name, ctx.channel.guild.name, ctx.channel.name))
    ...
    async def edit(self, ctx, messageId: int, *, arguments):
        ...
        print("command: /edit received from \'%s\' on guild \'%s\' channel \'%s\'" % (ctx.message.author.display_name, ctx.channel.guild.name, ctx.channel.name))
