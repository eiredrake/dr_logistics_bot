...
from src.constants.Constants import DATE_WITH_TWELVE_HOUR_TIME_FORMAT, A_EMOJI, B_EMOJI, CANCEL_EMOJI, OK_EMOJI, \
    ACCEPT_TRADE, CANCEL_TRADE, TradeResponse, NOTES_FIELD_NAME
...
class Trade_cog(commands.Cog, name='trade commands'):
    ...
    @commands.has_any_role('Admin', 'Full Ticket')
    ...
    async def trade(self, ctx: commands.Context, *, parameters: str):
        ...
        item_group = re.search(BASIC_STRING_FIELD_EXPRESSION % ITEMS_FIELD_NAME, parameters)
        ...
        if recipient_text.isnumeric():
            ...
            recipient_user = next((x for x in members if x.name == recipient_text or x.display_name == recipient_text), None)
        ...
        print('Trade attempt started between \'%s\' and \'%s\'' % (trade_request.player_a_name, trade_request.player_b_name))
        ...
        await player_a_message.edit(content='', embed=embed_data.to_embed(trade_request.player_a_dm_message.id), components=components_a)
        ...
        await player_b_message.edit(content='', embed=embed_data.to_embed(trade_request.player_b_dm_message.id), components=components_b)
    ...
    async def on_button_click(self, interaction: Interaction):
        ...
        trade_request = next((x for x in self.active_trades if x.player_a_dm_message.id == message_id or x.player_b_dm_message.id == message_id), None)
        if trade_request is not None:
            await interaction.respond(type=InteractionType.DeferredUpdateMessage, content='%s clicked' % component.label, hidden=False)
            print("Trade_cog: '%s' button pressed on message '%s' by '%s'... this one is MINE" % (component.label, message_id, clicking_user.display_name,))
            ...
            embed.add_field(name=trade_request.player_a.display_name, value=trade_request.player_a_response, inline=False)
            embed.add_field(name=trade_request.player_b.display_name, value=trade_request.player_b_response, inline=False)
    ...
    async def trade_timer_expired(self, trade_request: TradeRequest):
        ...
        print('Trade request between \'%s\' and \'%s\' expired' % (trade_request.player_a.nick, trade_request.player_b_name))
        ...
        trade_request.player_a_dm_message = await trade_request.player_a_dm_message.channel.send(content='incoming message')
        trade_request.player_b_dm_message = await trade_request.player_b_dm_message.channel.send(content='incoming message')

        await trade_request.player_a_dm_message.edit(content='', embed=new_embed.to_embed(trade_request.player_a_dm_message.id), components=[])
        await trade_request.player_b_dm_message.edit(content='', embed=new_embed.to_embed(trade_request.player_b_dm_message.id), components=[])
        await trade_request.origin_message.delete()
